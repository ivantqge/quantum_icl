...
\subsection{Experiment 3: Recent prompting techniques (CoT, Best-of-N) on Gemini-3-Flash}

We test two recent prompting techniques as additional levers on top of the
\texttt{structural\_retrieval\_plus\_feedback} baseline: (i) \textbf{chain-of-thought (CoT)}
-- the system prompt is augmented with an explicit 4-step reasoning instruction
(identify target structure, justify each gate, mentally execute, then emit
JSON); (ii) \textbf{Best-of-$N$ parallel sampling} -- temperature $0.4$--$0.5$ and
up to 5 attempts per task with feedback off (so attempts are independent
samples rather than refinements). Cost: \$2.74 total.

\begin{center}\small
\begin{tabular}{lrrrrr}
\toprule
\textbf{D-lite, Gemini-3-Flash} & \textbf{zero-shot} & \textbf{+fb} & \textbf{+struct} & \textbf{+struct+fb} \\
\midrule
default prompt          & 28 & 53 & 54 & 65 \\
+ chain-of-thought      & \textbf{47} & \textbf{60} & \textbf{56} & \textbf{71} \\
+ Best-of-5 (no fb)     & 53 & --- & --- & --- \\
+ CoT + Best-of-5 (no fb) & \textbf{58} & --- & --- & --- \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{tabular}{lrrrr}
\toprule
\textbf{D-mid, Gemini-3-Flash} & \textbf{zero-shot} & \textbf{+fb} & \textbf{+struct} & \textbf{+struct+fb} \\
\midrule
default prompt              & 9 & 9 & 15 & 17 \\
+ chain-of-thought          & 13 & 16 & 20 & 22 \\
+ Best-of-5 (no fb)         & 12 & --- & --- & --- \\
+ CoT + Best-of-5 + fb      & 18 & 17 & \textbf{32} & 28 \\
\bottomrule
\end{tabular}
\end{center}

\paragraph{Headline:} the three orthogonal levers \textbf{(retrieval, feedback, CoT/Best-of-N)} stack. On D-lite, the cumulative pipeline
\texttt{zero-shot} (28\%) $\to$ \texttt{+feedback+retrieval} (65\%) $\to$
\texttt{+CoT} (71\%) raises Gemini-3-Flash by +43 points. On the harder
D-mid, the same stack plus Best-of-$N$ goes from 9\% to 32\% -- nearly $4\times$.

\paragraph{Notable cross-effect.} CoT modestly helps Qwen-7B-SFT-3600 on D-lite
when combined with feedback (24 $\to$ 23 with CoT), suggesting the SFT
distribution biased the model toward direct-emission and partially blunted
CoT's effect. Open models likely benefit most when CoT is included in the
SFT training data, not just the eval prompt.

\subsection{Experiment 4: SFT data-size scan on Qwen2.5-7B}
...
\paragraph{F4: Chain-of-thought prompting is the largest single-lever gain for Gemini-3-Flash, and it stacks.}
On D-lite, switching from the default prompt to CoT lifts zero-shot from 28
to 47/100 -- a bigger delta than any of feedback, retrieval, or SFT alone. CoT
\emph{stacks} with structural+feedback (65 $\to$ 71). On the harder D-mid,
adding Best-of-$N$ to CoT+retrieval reaches 32/100, $\sim 4\times$ the default
zero-shot rate.

\paragraph{F5: SFT on verified synthetic data is non-monotone and can interfere with ICL.}
\begin{itemize}[leftmargin=*,topsep=2pt,itemsep=0pt]
...
\paragraph{F6: Tier difficulty calibration is critical for revealing ICL effects.}
